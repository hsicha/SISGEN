...
class Presentacion extends Controller{
    ...
    function index(){
        $this->view->title="Presentacion";
        $this->view->renderAdmin('presentacion/index');
    }
    function listarPresentacion(){
       $getData=$this->model->ListarPresentacion();
       print $getData;
    }
    ...
    function Registrar(){
        ...
        if(isset($_POST['idPresentacion'])&& empty($_POST['idPresentacion'])){
            $mensaje_OK=false;
            if(isset($_POST['descripcion'])){
                $data=array('CODIGO_PRESENTACION'=>$getId, 'DESCRIPCION'=>strtoupper(trim($descripcion_mayus)));
                $mensaje=$this->model->InsertarPresentacion($data);
            ...
            header("Location" .URL. "presentacion");
        }
    }
    function Actualizar(){
        if(isset($_POST['idPresentacion'])&& !empty($_POST['idPresentacion'])){
            $mensaje_OK=false;
            $data=array("CODIGO_PRESENTACION"=>trim($_POST['idPresentacion']),"DESCRIPCION"=>strtoupper(trim($_POST['descripcion'])));
            $mensaje=$this->model->ActualizarPresentacion($data);
            if($mensaje=="Presentación Actualizado"){
                $mensaje_OK=true;
                $mensajeRespuesta=$mensaje;
            }else{
                $mensajeRespuesta=$mensaje;
    ...
    public function ExportarExcelPresentacion(){
        ...
        ->setKeywords('Lista de Presentación')
        ->setCategory('Presentación');
    
        // contenido del document excel
        $hoja = $documento->getActiveSheet();
    
        $titulo="LISTADO DE PRESETACION";
        $styleCenter=array
        (
            'alignment'=>array(
        ...
        $columnas=array('A','B');
        $encabezado=array('N°','PRESENTACION');
        for($i=3; $i<4;$i++){
        ...
        $presentacion=$this->model->ExportarExcel();
        $i=3;
        $cant=0;
       ...
       //  $hoja->setAutoFilter("B3:B3");
        ...
        //documento a guardar
        $tituloDoc="Listado de Presentacion.xls";
